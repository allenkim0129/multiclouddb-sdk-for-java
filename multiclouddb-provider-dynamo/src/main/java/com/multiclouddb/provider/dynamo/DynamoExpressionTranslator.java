...
public final class DynamoExpressionTranslator implements ExpressionTranslator {
        ...
        switch (func.function()) {
            ...
            case FIELD_EXISTS -> {
                ...
                if (!func.arguments().isEmpty() && func.arguments().get(0) instanceof FieldRef field) {
                    sb.append(fieldRef(field.name())).append(" IS NOT MISSING");
                }
            }
            case STRING_LENGTH -> {
     ...
     * quotes its own partition key and compiles patch paths to
     * {@code ExpressionAttributeNames} placeholders for exactly this reason, so
     * leaving caller-supplied query fields bare meant a field could be patched but
     * never queried.
     * <p>
