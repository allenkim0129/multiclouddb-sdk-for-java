...
- Declares `PARTIAL_UPDATE` supported and `PARTIAL_UPDATE_EXTENDED_PAYLOAD` unsupported because either the 4,096-byte expression envelope or state-dependent 409,600-byte result-item envelope can bind before the common 408,576-byte field-map limit.
