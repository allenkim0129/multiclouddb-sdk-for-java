...
- Declares `PARTIAL_UPDATE` supported and `PARTIAL_UPDATE_EXTENDED_PAYLOAD` unsupported because either the 4 KiB (4,096 bytes) expression envelope or state-dependent 400 KiB (409,600 bytes) result-item envelope can bind before the common 399 KiB (408,576 bytes) field-map limit.
