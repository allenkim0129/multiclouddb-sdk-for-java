...
- `DynamoCapabilities` declares **`EXACT_FRACTIONAL_INCREMENT` as supported**. DynamoDB evaluates `field = field + :delta` server-side in its `N` type, which is exact decimal arithmetic with 38 significant digits, so accumulated fractional results carry no binary rounding drift. The declaration is informational and lets callers that require bit-identical fractional totals branch explicitly.
- `DynamoCapabilities` declares **`PATCH_PRESERVES_TTL` as supported** because PATCH never rewrites the absolute `ttlExpiry` attribute.
- DynamoDB numeric reads now use a `BigIntegerNode` fallback for integral values wider than signed 64-bit, so a wide integer written by PATCH remains readable instead of leaking `NumberFormatException`.
