...
- Accepted `INCREMENT` deltas are normalized by the shared portable numeric domain before construction of DynamoDB's numeric attribute, enforcing signed-64-bit integral results and the canonical finite fractional representation.
- The patch `ConditionExpression` adds `attribute_not_exists(ttlExpiry)`, so patching an item that carries an SDK-managed expiry fails with `UNSUPPORTED_CAPABILITY` rather than succeeding. `UpdateItem` alone would have preserved the absolute `ttlExpiry` attribute, but Cosmos DB cannot preserve its relative `ttl`, so the portable contract rejects the case on both providers rather than letting the same call keep an expiry on one and fail on the other.
- DynamoDB numeric reads now use a `BigIntegerNode` fallback for integral values wider than signed 64-bit, so an item carrying such a value remains readable instead of leaking `NumberFormatException`. (PATCH itself no longer writes values outside the portable numeric domain; the fallback covers items written by other clients or by earlier SDK versions.)
...
- **Numbers stored in exponent notation no longer make an item permanently unreadable.** DynamoDB's `N` type accepts forms such as `1E+10` and `1E-130` (the documented numeric floor), but `DynamoItemMapper` parsed integral-looking values straight through `BigInteger`, which rejects an exponent and threw a raw `NumberFormatException` on **every** subsequent read of that item - not a `MulticloudDbException`, and not recoverable by re-reading. Values are now parsed through `BigDecimal` first, yielding an exact integer when one exists and a decimal otherwise.
- **Portable query filters on fields whose names are DynamoDB reserved words no longer produce a syntax error.** `DynamoExpressionTranslator` emitted bare PartiQL identifiers, so a filter on a field named `value` (or any of DynamoDB's several hundred reserved words) failed at parse time. Field references are now emitted as quoted identifiers (`"value"`), one quoted segment per path component, with embedded `"` doubled. The `UpdateItem` path already used `ExpressionAttributeNames` placeholders and was unaffected; this aligns the PartiQL query path with it.
