...
- Strict-path, nested-path, and numeric-result state is validated by a point read before the native patch. The write itself is then guarded by Cosmos's **server-side conditional patch** (`CosmosPatchItemRequestOptions.setFilterPredicate`) — a *path-scoped* precondition of the form `FROM c WHERE IS_DEFINED(c["field"]) AND ...` — attached **only** for `REPLACE`, `REMOVE`, and nested non-increment operations, whose native Cosmos translation cannot enforce the portable contract by itself. `INCREMENT` contributes no predicate term at any depth: `CosmosPatchOperations.increment` is evaluated atomically server-side, so concurrent increments all land, matching DynamoDB's `SET x = x + :v` and Spanner's retryable read-write transaction.
- Patch-specific error normalisation returns `NOT_FOUND` for a missing required path and `INVALID_REQUEST` for a nonnumeric target or proven signed-64 result overflow during that pre-read. A HTTP **412** from the conditional patch means the addressed path was absent when the server evaluated the write, and is normalised to `NOT_FOUND` — the same category DynamoDB and Spanner report for that state. The exact Cosmos emulator 412 behavior remains unverified pending T192.
