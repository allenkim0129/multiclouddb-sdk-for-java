...
- Accepted updates issue one Cosmos SDK request. Wide requests are rejected before I/O above 100 batch operations or 2 MiB (2,097,152 serialized bytes); RU cost grows with patch operations/chunks.
...
- Declares `PARTIAL_UPDATE` supported and `PARTIAL_UPDATE_EXTENDED_PAYLOAD` unsupported because native request or resulting-item envelopes can bind before the common 399 KiB (408,576 bytes) limit.
