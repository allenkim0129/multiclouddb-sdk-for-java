...
- **Portable `ORDER BY` cross-type ranking now matches Cosmos.** Values of different JSON types were ranked `number < string < boolean < null/absent`; they are now ranked `null/absent < boolean < number < string < array < object`, reproducing Cosmos NoSQL's documented total order (`undefined < null < boolean < number < string < array < object`). Arrays and objects are ranked explicitly rather than folded in with null, which had placed Cosmos's two highest kinds on Spanner's lowest rank. DynamoDB declares `ORDER_BY` unsupported, so Cosmos and Spanner are the only two providers on this surface and their disagreement was an ungated divergence. A query that sorts a field holding mixed JSON types therefore returns a different row order than before; ordering within a single type (numeric for numbers, lexicographic for strings, `FALSE` before `TRUE` for booleans) and the `partitionKey` / `sortKey` tiebreakers are unchanged, and `DESC` remains the exact reverse of `ASC`.
