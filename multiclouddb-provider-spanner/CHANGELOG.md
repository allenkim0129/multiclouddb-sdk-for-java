...
- **Through `MulticloudDbClient`, `update()` is unavailable for Spanner in this release.** `SpannerCapabilities` explicitly declares `PARTIAL_UPDATE` unsupported, so otherwise-valid calls fail before provider delegation with non-retryable `UNSUPPORTED_CAPABILITY` (`capability=partial_update`). The adapter's existing read-modify-write implementation is not exposed through the portable client.
