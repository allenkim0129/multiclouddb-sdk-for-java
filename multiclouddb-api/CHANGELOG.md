...
- Clarified that `CapabilitySet` normalization is capability-specific: omitted `PARTIAL_UPDATE`, `PARTIAL_UPDATE_EXTENDED_RESULT_SIZE`, and `PARTIAL_UPDATE_PRESERVES_TTL_EXPIRY` declarations receive unsupported defaults, but unrelated omitted well-known names remain absent. Each built-in provider exposes 20 effective rows; Cosmos DB and DynamoDB declare all 20, while Spanner declares 17 and receives the three defaults.

- `MulticloudDbClient.update()` now specifies shallow top-level set/replace semantics: supplied fields are replaced, omitted fields remain, map/list values replace their complete top-level value, and a missing item returns `NOT_FOUND` without create. Non-reserved case-distinct names such as `foo` and `Foo` remain separate even in one atomic update. Replaying absolute assignments is idempotent for logical fields, not provider metadata or TTL timing. Base `PARTIAL_UPDATE` does not promise fixed absolute TTL expiry; callers requiring it must also check `PARTIAL_UPDATE_PRESERVES_TTL_EXPIRY`. The synchronous one-write/no-read-merge execution contract is unchanged.
- `DocumentResult.document()` and `QueryPage.items()` now explicitly require providers to omit adapter-injected identity, TTL, and system-metadata fields; requested read metadata remains separate in `DocumentMetadata`.
...
- Shared preflight snapshots the supplied top-level `Map` entries before validation, so class-level map serializers cannot rewrite validated field names or counts. The bounded iterative graph inspection rejects cycles and non-collection `Iterable` values without recursive stack growth or unbounded eager scheduling; serialized JSON output is capped at 390 KiB while it is produced, and `char[]` follows Jackson string semantics.
