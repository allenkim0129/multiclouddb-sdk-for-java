...
- PATCH now has one central portable numeric domain, applied to the value written by `SET` / `REPLACE` (including numbers nested inside an object or array operand) as well as to an `INCREMENT` delta: integral values and any resulting value must fit signed 64-bit range; non-zero fractional values must be at least `1E-130` in magnitude (DynamoDB's numeric floor), no larger than 9,007,199,254,740,991, finite, and round-trip through an IEEE-754 `double` without decimal precision loss. Accepted values are normalized before dispatch, and integral-result overflow is rejected atomically as `INVALID_REQUEST`, preventing provider-specific narrowing or precision drift. Bounding written values matters as much as bounding deltas: DynamoDB stores integers wider than signed 64-bit exactly while Cosmos DB rounds them to the nearest IEEE-754 `double`, so an unbounded `SET` of `2^63` would have stored `9223372036854775808` on one provider and `9223372036854776000` on the other.
...
- **Portable PATCH retry contract.** `CONFLICT` is the only PATCH category reported with `MulticloudDbError.retryable() == true`: a patch is atomic, so a rejected conditional write applied no operation and re-issuing the identical request cannot double-apply an `INCREMENT`. Every other PATCH category (`NOT_FOUND`, `INVALID_REQUEST`, `UNSUPPORTED_CAPABILITY`, `CLIENT_CLOSED`) names a deterministic cause an identical retry would reproduce and is reported as non-retryable. The SPI contract requires this of every adapter, and `PatchConformanceTest` asserts both halves on each provider, so a retry layer written against `retryable()` behaves the same everywhere. Note this differs from a create-duplicate `CONFLICT`, which stays non-retryable because the conflicting item genuinely exists.
...
- `PatchNumericDomain` now documents **why an `INCREMENT` delta must be whole-valued, and why that is decided by the normalized value rather than by the Java type you passed**. Normalization folds any whole-valued `Float` / `Double` / `BigDecimal` to a `Long`, so `increment("/v", 1.0)` and `increment("/v", 1)` are the same portable request and both succeed, while `increment("/v", 1.5)` is `INVALID_REQUEST` on every provider. The result of an accepted increment is bounded to signed 64-bit, so on a field holding `1e300` even `increment("/v", 1)` is `INVALID_REQUEST`. Use `PatchNumericDomain.isIntegralDelta(delta)` to check a delta before dispatch.
