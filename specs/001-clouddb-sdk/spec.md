...
**Why this priority**: Enterprise customers operate in multi-region deployments where global strong consistency is too expensive for most reads, but eventual consistency is too weak for critical data. An intermediate level that provides strong reads within the local region satisfies most use cases. Cosmos DB Session/Bounded Staleness, DynamoDB strongly consistent reads (region-local), and Spanner strong reads (globally linearizable; may exceed the LOCAL_QUORUM semantic) all approximate this semantic.
...
- **FR-163**: The SDK MUST define a portable, provider-neutral read consistency enumeration with at minimum the levels `EVENTUAL`, `LOCAL_QUORUM` (strong-within-region), and `GLOBAL_STRONG` (strong across regions). This enumeration is the portable surface for expressing read consistency intent; the legacy `STRONG`/`EVENTUAL` override remains supported as a backward-compatible subset.
...
- **FR-165**: Each provider adapter MUST map every portable consistency level to the closest native guarantee and MUST document the mapping. Indicative mappings: `EVENTUAL` → eventual/relaxed reads; `LOCAL_QUORUM` → Cosmos DB Session/Bounded Staleness, DynamoDB strongly-consistent (region-local) read, Spanner strong read (external consistency / linearizable; may exceed local-quorum semantics); `GLOBAL_STRONG` → Cosmos DB strong (multi-region), Spanner strong read (external consistency / linearizable). DynamoDB does not natively support cross-region strong reads and MUST declare `GLOBAL_STRONG` unsupported (capability-gated).
...
| Portable consistency levels (`EVENTUAL` / `LOCAL_QUORUM` / `GLOBAL_STRONG`) *(preferred; `STRONG`/`EVENTUAL` remains as a legacy/back-compat subset)* | Cosmos DB, DynamoDB, Spanner (per-provider native mapping; capability-gated per level) |
