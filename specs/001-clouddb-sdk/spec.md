...
8. **Given** an item with SDK-managed row-level TTL, **When** PATCH is requested, **Then** a provider advertising `PATCH_PRESERVES_TTL` keeps the same absolute expiry, while a provider not advertising it raises `UNSUPPORTED_CAPABILITY` without mutating the item.
9. **Given** a non-zero fractional increment below `1E-130` or a wide integral value written by `SET`, **When** the request is submitted and read back, **Then** the sub-floor delta is rejected before provider I/O and the accepted wide integer remains readable on every provider advertising PATCH.
...
- **FR-186**: On a provider that advertises PATCH, `INCREMENT` MUST evaluate its delta within the same atomic request or equivalent atomic transaction, so concurrent increments do not lose updates. Integral operands and their resulting value MUST fit signed 64-bit range. Non-zero fractional operands MUST have magnitude from `1E-130` through 9,007,199,254,740,991, be finite, and round-trip through an IEEE-754 `double` without decimal precision loss. Invalid operands MUST be rejected with `INVALID_REQUEST` before provider dispatch; an integral-result overflow MUST be rejected atomically with the write as `INVALID_REQUEST`, without misclassifying a missing document or field as overflow.
- **FR-186a**: Providers MUST declare `EXACT_FRACTIONAL_INCREMENT` alongside PATCH. DynamoDB declares it supported because its numeric type uses exact decimal arithmetic; Cosmos DB declares it unsupported because fractional increments use IEEE-754 binary64. Spanner declares it unsupported while PATCH itself is unavailable, and any future implementation MUST declare the capability according to its actual arithmetic. The capability is informational for PATCH-supporting providers: an in-domain fractional delta MUST NOT be rejected because exact fractional accumulation is unsupported, and the SDK MUST NOT emulate exact-decimal accumulation client-side because doing so would forfeit FR-186 atomicity.
...
- **FR-190**: `OperationOptions.ttlSeconds()` MUST be ignored uniformly by providers that advertise PATCH and MUST NOT create or reset expiry. Providers MUST declare `PATCH_PRESERVES_TTL`: a supporting provider preserves an existing SDK-managed absolute expiry, while an unsupported provider MUST reject a TTL-bearing item with `UNSUPPORTED_CAPABILITY` rather than silently extending or removing expiry. A deterministic serialized representation of every operation's type, path, and optional value MUST be subject to a 399 KB (408,576-byte) request-envelope limit; `REMOVE` contributes its type and path even though it has no value. This validation does not promise that the resulting stored item fits every provider's native post-image limit.
