...
As an application developer, I can update individual fields of an existing document - including atomically incrementing a counter - without reading, mutating and rewriting the whole document, and the operation behaves identically on every provider that advertises PATCH support.

**Why this priority**: Read-modify-write is a common source of lost updates in application code, and it is exactly what a portable SDK should make unnecessary. Cosmos DB and DynamoDB expose native partial writes. Spanner does not ship PATCH in this scope and declares it unsupported. A provider-neutral contract plus explicit capability declarations let a future Spanner implementation be added without changing the API or silently weakening semantics.

**Independent Test**: A conformance corpus applies each operation type against a seeded document on every provider advertising PATCH and asserts identical outcomes: identical resulting documents, identical error categories for missing documents and missing fields, and all-or-nothing application of a multi-operation call. Providers without PATCH are tested for a structured `UNSUPPORTED_CAPABILITY` failure before provider I/O. Nested-path behavior is asserted against `NESTED_PATCH`.

**Acceptance Scenarios**:

1. **Given** a provider that advertises PATCH and an existing document, **When** an application submits a list of `SET` / `REPLACE` / `REMOVE` / `INCREMENT` operations addressed by JSON Pointer, **Then** every operation is applied atomically in a single native request or equivalent atomic provider transaction, and either all of them take effect or none does.
2. **Given** a provider that advertises PATCH and a key that does not exist, **When** a patch is submitted, **Then** the SDK raises `NOT_FOUND` and no document is created.
3. **Given** a provider that advertises PATCH and an existing document, **When** `REPLACE`, `REMOVE` or `INCREMENT` targets an absent field, **Then** the SDK raises `NOT_FOUND` and no part of the patch is applied.
4. **Given** concurrent writers on a provider that advertises PATCH, **When** each uses `INCREMENT`, **Then** the delta is evaluated within the same atomic request or transaction and no update is lost.
5. **Given** a provider that advertises PATCH and a request that violates the portable contract - more than the maximum number of operations, overlapping paths, an array-index segment, a JSON Pointer escape, or a key/TTL field name - **When** it is submitted, **Then** the SDK raises `INVALID_REQUEST` before provider I/O.
6. **Given** a provider that advertises PATCH but not NESTED_PATCH, **When** a patch path has more than one segment, **Then** the SDK raises `UNSUPPORTED_CAPABILITY` before execution; it never rewrites the parent field and never silently ignores the operation.
7. **Given** a provider that does not advertise PATCH, **When** any patch is submitted, **Then** the SDK raises `UNSUPPORTED_CAPABILITY` before issuing a provider request.
8. **Given** an item with SDK-managed row-level TTL, **When** PATCH is requested, **Then** every provider advertising PATCH raises `UNSUPPORTED_CAPABILITY` without mutating the item, rather than one keeping the original expiry and another restarting it.
9. **Given** a non-zero fractional value below `1E-130` written by `SET` or a wide integral value written by `SET`, **When** the request is submitted and read back, **Then** the sub-floor value is rejected before provider I/O and the accepted wide integer remains readable on every provider advertising PATCH.
10. **Given** a provider that advertises PATCH, **When** an `INCREMENT` delta does not normalize to a whole number, **Then** the SDK raises `INVALID_REQUEST` before provider I/O and the stored value is unchanged; a whole-valued floating delta such as `1.0` is accepted and behaves identically to `1`.
...
- All operations (`read`, `upsert`, `patch`, `delete`, `query`, `ensureDatabase`,
  `ensureContainer`, `provisionSchema`) return results synchronously.
...
- **FR-181**: The SDK MUST expose a provider-neutral `patch` operation on the public client interface that applies an ordered list of field-level operations - `SET`, `REPLACE`, `REMOVE`, `INCREMENT` - to an existing document, addressed by JSON Pointer paths. Each provider MUST explicitly declare PATCH supported or unsupported. Cosmos DB and DynamoDB MUST use their native partial-write primitives (`patchItem`, `UpdateItem`). Spanner is unsupported in this scope and MUST fail with `UNSUPPORTED_CAPABILITY` before provider I/O. Any future implementation MUST satisfy this same contract and conformance suite before advertising support; no adapter may expose a non-transactional client-side read-modify-write window.
- **FR-182**: On a provider that advertises PATCH, all operations in one `patch` call MUST be applied atomically - either every operation takes effect or none does - in a single native request or an equivalent atomic provider transaction.
- **FR-183**: On a provider that advertises PATCH, the target document MUST already exist and a missing document MUST fail with `NOT_FOUND`. `patch` MUST NOT create a document; creation remains the responsibility of `create`/`upsert`.
- **FR-184**: The SDK MUST enforce a uniform maximum number of operations per call (the lowest native limit across providers, exposed as a public constant; Cosmos DB caps a single-document patch at [10 operations](https://learn.microsoft.com/en-us/azure/cosmos-db/partial-document-update#supported-modes) while DynamoDB has no equivalent operation-count cap) and MUST reject an over-limit list with `INVALID_REQUEST` before any provider call, so a patch that succeeds on one provider cannot fail on another.
- **FR-185**: On a provider that advertises PATCH, `REPLACE`, `REMOVE` and `INCREMENT` MUST require the addressed field to exist and MUST fail with `NOT_FOUND` when it does not. `SET` MUST create the field or overwrite it if present, and MUST NOT create missing intermediate objects. A more permissive native primitive MUST be constrained rather than exposing the divergence.
- **FR-186**: On a provider that advertises PATCH, `INCREMENT` MUST evaluate its delta within the same atomic request or equivalent atomic transaction, so concurrent increments do not lose updates. The delta and its resulting value MUST fit signed 64-bit range. Invalid operands MUST be rejected with `INVALID_REQUEST` before provider dispatch; an integral-result overflow MUST be rejected atomically with the write as `INVALID_REQUEST`, without misclassifying a missing document or field as overflow.
- **FR-186a**: An `INCREMENT` delta MUST normalize to a whole number; a fractional delta MUST be rejected with `INVALID_REQUEST` before provider dispatch, uniformly on every provider that advertises PATCH. Providers do not agree on fractional accumulation - DynamoDB adds in its exact-decimal numeric type while Cosmos DB adds in IEEE-754 binary64, so `0.1` incremented by `0.2` would store `0.3` on one and `0.30000000000000004` on the other. The SDK MUST NOT emulate exact-decimal accumulation client-side, because reading the current value would forfeit the FR-186 atomicity guarantee, and MUST NOT declare the divergence as a capability, because a portable call MUST NOT store different data depending on the provider. Normalization folds whole-valued `Float` / `Double` / `BigDecimal` operands to integers, so `increment(path, 1.0)` MUST be accepted and behave identically to `increment(path, 1)`.
- **FR-186b**: On a provider that advertises PATCH, numbers written by `SET` and `REPLACE` - including numbers nested inside an object or array operand - MUST satisfy the portable numeric domain - integral values within signed 64-bit range, and non-zero fractional values with magnitude from `1E-130` through 9,007,199,254,740,991, finite, and round-tripping through an IEEE-754 `double` without decimal precision loss -, and MUST be rejected with `INVALID_REQUEST` before provider dispatch when they do not. Without this bound the same portable write would store provider-dependent data: DynamoDB's numeric type preserves integers wider than signed 64-bit exactly, while Cosmos DB stores JSON numbers and rounds anything wider than `2^53 - 1` to the nearest IEEE-754 binary64 value, so writing `2^63` would store `9223372036854775808` on one provider and `9223372036854776000` on the other.
- **FR-187**: Paths within a single `patch` call MUST be disjoint. Duplicate paths, case-only aliases, and any path that is a prefix of another MUST be rejected with `INVALID_REQUEST`, because providers can differ in path case handling and whether operands resolve against the pre-update or progressively updated document.
- **FR-188**: The SDK MUST reject, with `INVALID_REQUEST` and before provider I/O on a PATCH-supporting adapter, path forms whose native interpretations differ or that address SDK-owned state: paths not rooted at `/`, empty path segments, JSON Pointer `~` escapes, array-index segments, and key, TTL or SDK-reserved field names (including `data`).
- **FR-188a**: The SDK MUST reject a top-level document field named `data`, case-insensitively, with `INVALID_REQUEST` before dispatch from `create`, `update`, and `upsert` on every provider. Patch continues to reject the `/data` root through FR-188. This is an SDK metadata reservation and cannot be used as an ordinary document field.
...
- **FR-190**: `OperationOptions.ttlSeconds()` MUST be ignored uniformly by providers that advertise PATCH and MUST NOT create or reset expiry. A patch targeting a document that already carries an SDK-managed expiry MUST be rejected with `UNSUPPORTED_CAPABILITY` before any mutation, uniformly on every provider that advertises PATCH, rather than silently extending or removing that expiry. Cosmos DB's `ttl` is a relative countdown that a native patch restarts by advancing `_ts`, so it cannot preserve the original expiry; DynamoDB's `ttlExpiry` is absolute and would survive, so DynamoDB MUST be constrained up to reject the case as well rather than letting the same portable call keep an expiry on one provider and change it on another. A deterministic serialized representation of every operation's type, path, and optional value MUST be subject to a 399 KB (408,576-byte) request-envelope limit; `REMOVE` contributes its type and path even though it has no value. This validation does not promise that the resulting stored item fits every provider's native post-image limit.
- **FR-191**: The conformance suite MUST verify every behaviour above against each provider advertising PATCH and MUST verify structured pre-dispatch capability failure for unsupported providers. Both branches of `NESTED_PATCH` MUST remain covered as implementations evolve.
...
- [ ] Cosmos DB and DynamoDB use native partial writes; Spanner declares PATCH unsupported before provider I/O, and any future implementation must pass the same contract before advertising support.
- [ ] On every provider advertising PATCH, all operations in a call apply atomically, and a missing document fails with `NOT_FOUND` rather than being created.
- [ ] On every provider advertising PATCH, `REPLACE` / `REMOVE` / `INCREMENT` fail with `NOT_FOUND` on a missing field; `SET` creates-or-overwrites but never creates intermediate objects.
- [ ] Operation-count, path-disjointness and path-form rules are validated before provider I/O by every adapter advertising PATCH and produce `INVALID_REQUEST` identically.
