...
- [x] T057 [P] [US1b] Implement CosmosExpressionTranslator in `multiclouddb-provider-cosmos/src/main/java/com/multiclouddb/provider/cosmos/CosmosExpressionTranslator.java` (AST → Cosmos SQL WHERE clause; adds `c.` prefix to field refs; keeps `@paramName` parameters; translates functions: starts_with→STARTSWITH, contains→CONTAINS, field_exists→`IS_DEFINED AND NOT IS_NULL`, string_length→LENGTH, collection_size→ARRAY_LENGTH; generates full statement `SELECT * FROM c WHERE ...`)
- [x] T058 [P] [US1b] Implement DynamoExpressionTranslator in `multiclouddb-provider-dynamo/src/main/java/com/multiclouddb/provider/dynamo/DynamoExpressionTranslator.java` (AST → DynamoDB PartiQL WHERE clause; double-quotes table name; converts `@paramName` to positional `?` parameters and builds ordered parameter list; translates functions: starts_with→begins_with, contains→contains, field_exists→`IS NOT MISSING AND IS NOT NULL`, string_length→char_length, collection_size→size; handles reserved word escaping with double-quotes)
- [x] T059 [P] [US1b] Implement SpannerExpressionTranslator in `multiclouddb-provider-spanner/src/main/java/com/multiclouddb/provider/spanner/SpannerExpressionTranslator.java` (AST → Spanner GoogleSQL WHERE clause; bare field names; keeps `@paramName` parameters; translates functions: starts_with→STARTS_WITH, contains→STRPOS(field,value)>0, field_exists→JSON non-null type test, scalar comparisons→JSON-type-guarded LAX coercions, string_length→CHAR_LENGTH, collection_size→ARRAY_LENGTH; uses the complete envelope per row and only falls back to physical columns for legacy/malformed rows; generates full statement `SELECT * FROM <table> WHERE ...`)
...
## Phase 20: User Story 28 — Portable Field-Level Patch (Priority: P1)

**Goal**: A portable `patch(addr, key, List<PatchOperation>)` that uses native partial writes on Cosmos DB and DynamoDB and a retryable Spanner document-envelope transaction, without exposing a non-transactional client-side read-modify-write. The v1 contract is deliberately narrow so the implementations cannot diverge (FR-181–FR-192); the single genuine asymmetry, nested-path addressing, is declared through `Capability.NESTED_PATCH` rather than tolerated silently.

- [x] T182 [US28] Public API type `PatchOperation` — `Type` enum (`SET`, `REPLACE`, `REMOVE`, `INCREMENT`), static factories, JSON Pointer path accessors (`pathSegments()`, `rootField()`, `isNested()`), value semantics.
  File: `multiclouddb-api/src/main/java/com/multiclouddb/api/PatchOperation.java`
- [x] T183 [US28] `PatchValidator` plus shared document-field validation — the portable contract is applied before provider dispatch: operation-count cap, case-insensitive disjoint-path check (duplicates, aliases, and prefix overlaps), path-form rules (must be rooted at `/`, no empty segments, no `~` escapes, no array-index segments, no key/TTL/SDK-reserved roots including `data`), case-insensitive `data` rejection on `create`/`update`/`upsert`, central signed-64-bit integral result and canonical finite IEEE-754 fractional `INCREMENT` domain, and the 399 KB (408,576-byte) deterministic request representation containing every operation type/path/optional value. Each rule's Javadoc names the divergence it prevents.
  File: `multiclouddb-api/src/main/java/com/multiclouddb/api/internal/PatchValidator.java`
- [x] T184 [US28] `MulticloudDbClient.patch(...)` (with and without `OperationOptions`) plus `MAX_PATCH_OPERATIONS = 10`, and the `MulticloudDbProviderClient` SPI mirror defaulting to `UNSUPPORTED_CAPABILITY` so existing adapters compile unchanged. The shared SPI `validatePatchRequest(...)` performs portable validation before PATCH/NESTED_PATCH capability checks; the facade delegates through it and every provider invokes it after its close guard, so direct SPI callers receive the same errors. `OperationNames.PATCH` flows into diagnostics and error envelopes.
  Files: `multiclouddb-api/src/main/java/com/multiclouddb/api/MulticloudDbClient.java`, `.../spi/MulticloudDbProviderClient.java`, `.../internal/DefaultMulticloudDbClient.java`, `.../OperationNames.java`
- [x] T185 [P] [US28] Cosmos provider: `patch()` via `CosmosContainer.patchItem(...)`. Requests with a strict (`REPLACE`/`REMOVE`/`INCREMENT`) or nested path point-read and validate required-path and signed-64 numeric state. Every addressed path also contributes a server-side, path-scoped filter predicate instead of an `If-Match` ETag guard: an `IS_DEFINED` existence check, plus — for an `INCREMENT` with an integral delta — a `BETWEEN` bound on the current value so the portable signed-64 result range is enforced atomically with the write. Because every term is path-scoped, a concurrent write to an unaddressed field cannot falsify it and concurrent increments of an in-range counter all land instead of failing as non-retryable `CONFLICT`s. Missing path/document is `NOT_FOUND` and nonnumeric/overflow state is `INVALID_REQUEST`; a failed predicate (HTTP 412) and an untyped `400` are both classified from a re-read of current state, yielding the same `NOT_FOUND` / `INVALID_REQUEST` / `CONFLICT` categories DynamoDB derives from its before-image. Documented residual: a rejection whose cause current state cannot prove falls back to `CONFLICT` for a 412 and to `INVALID_REQUEST` for a 400.
  Files: `multiclouddb-provider-cosmos/src/main/java/com/multiclouddb/provider/cosmos/CosmosProviderClient.java`, `.../CosmosConstants.java`
- [x] T186 [P] [US28] Dynamo provider: `patch()` compiles the operation list into a single `UpdateItem` `UpdateExpression` (`SET ... REMOVE ...`) with a `ConditionExpression` asserting `attribute_exists(partitionKey)`, strict paths, and signed-64 integral-result bounds — without it `UpdateItem` would create a missing item, allow arbitrary-precision results, and a native `REMOVE` would silently no-op. `ReturnValuesOnConditionCheckFailure.ALL_OLD` classifies missing, nonnumeric, and overflow failures correctly. Path segments always become `ExpressionAttributeNames` placeholders (reserved-word safety); `ExpressionAttributeValues` is omitted entirely for a `REMOVE`-only patch. New `DynamoItemMapper.objectToAttributeValue(Object)` maps `Map`/`List` operands to real `M` / `L` attributes.
  Files: `multiclouddb-provider-dynamo/src/main/java/com/multiclouddb/provider/dynamo/DynamoProviderClient.java`, `.../DynamoItemMapper.java`
- [x] T187 [P] [US28] Spanner provider: `patch()` reads and updates the standard `data` document envelope inside one retryable `readWriteTransaction`, keeping existence checks and increments atomic while supporting dynamic top-level fields on the standard schema. Portable expression translation uses GoogleSQL JSON functions over that authoritative envelope with a physical-column fallback for legacy rows, so query and read state agree. Legacy fixed-column rows remain readable; full writes mirror only runtime-compatible physical values and explicitly clear omitted, null, or incompatible mirrors to typed nulls. Nested paths are rejected before the transaction opens as deferred nested JSON traversal scope; in-transaction failures are raised as `SpannerException`s so the error mapper produces `NOT_FOUND`/`INVALID_REQUEST` rather than `PROVIDER_ERROR`.
  File: `multiclouddb-provider-spanner/src/main/java/com/multiclouddb/provider/spanner/SpannerProviderClient.java`
- [x] T188 [US28] Capability declarations: `Capability.PATCH`, `Capability.NESTED_PATCH`, and `Capability.EXACT_FRACTIONAL_INCREMENT` with supported/unsupported singletons; declared on all three `*Capabilities` classes with qualified, provider-shaped cost notes. Spanner declares `NESTED_PATCH` **unsupported** because nested JSON traversal is deferred from the v1 compatibility scope. `EXACT_FRACTIONAL_INCREMENT` is supported on DynamoDB only (exact-decimal `N` arithmetic) and unsupported on Cosmos and Spanner (IEEE-754 binary64); it is informational and never causes an `UNSUPPORTED_CAPABILITY` rejection. Capability count 17 → 20 in `CapabilitiesConformanceTest`.
  Files: `multiclouddb-api/src/main/java/com/multiclouddb/api/Capability.java`, `multiclouddb-provider-{cosmos,dynamo,spanner}/.../*Capabilities.java`, `multiclouddb-conformance/src/test/java/com/multiclouddb/conformance/us2/CapabilitiesConformanceTest.java`
- [x] T189 [US28] Unit coverage in `multiclouddb-api`: `PatchValidatorTest` (every rejection rule) and `DefaultMulticloudDbClientPatchTest` (both capability gates, SPI default, delegation, error attribution).
  Files: `multiclouddb-api/src/test/java/com/multiclouddb/api/internal/PatchValidatorTest.java`, `.../DefaultMulticloudDbClientPatchTest.java`
- [x] T190 [US28] Conformance suite `us28`: provider-agnostic `PatchConformanceTest` asserting identical outcomes for every operation type, missing-document and missing-field `NOT_FOUND`, all-or-nothing atomicity, and every validator rule with unchanged-state assertions: null/empty lists and entries, count limit, duplicate and ancestor overlap, all reserved roots plus case variants, path form/array/escape rules, numeric-domain boundaries, and oversized payload/`REMOVE` paths. It also covers signed-64 positive and negative result-overflow rejection, TTL-ignored, whole and fractional numeric parity, a patch-then-portable-query assertion for fractional and dynamic values, `INCREMENT` on a non-numeric field, object-value round-trip, typed and dynamic `SET null` vs `REMOVE`, concurrent-increment lost-update safety, and **both** locked branches of `NESTED_PATCH`; every seeded key is deleted in `@AfterEach` without masking an assertion failure. Per-provider concrete subclasses exist for Cosmos, Dynamo and Spanner.
  Files: `multiclouddb-conformance/src/test/java/com/multiclouddb/conformance/us28/PatchConformanceTest.java` + per-provider `*PatchConformanceTest.java`
- [x] T191 [US28] Documentation: `docs/guide.md` → *patch - Field-Level Partial Update* section (operations table, v1 restrictions with the reason for each, direct-SPI guard, `data` migration, nested-path capability check, per-provider behavior); `docs/compatibility.md` → CRUD-table row, *Patch Semantics* section (native-or-equivalent capability declarations, qualified cost model, path-scoped filter-predicate 412 classification); `docs/architecture.md` public API/SPI inventory; `[Unreleased]` entries in `docs/changelog.md` and all four per-module `CHANGELOG.md` files.
  Files: `README.md`, `docs/guide.md`, `docs/compatibility.md`, `docs/architecture.md`, `docs/api-reference.md`, `docs/changelog.md`, `multiclouddb-api/CHANGELOG.md`, `multiclouddb-provider-{cosmos,dynamo,spanner}/CHANGELOG.md`
- [ ] T192 [US28] Run the `us28` conformance suite against the live emulators (`-Pemulator-cosmos`, `-Pemulator-dynamo`, `-Pemulator-spanner`). The unit suite is not emulator certification; emulator-tagged assertions remain unverified — in particular that the Cosmos emulator supports the Patch API and returns the expected **412** when the path-scoped filter predicate is falsified after the validating point read, and that the adapter's re-read classifies it as `NOT_FOUND` / `INVALID_REQUEST` / `CONFLICT`.
  **Partial status — Spanner only.** The suite *has* been run against the Spanner emulator (`-Pemulator-spanner`): **119 tests, 0 failures, 3 skipped**, including `SpannerPatchConformanceTest` **47/47**. That run was not a formality — it caught a defect the unit suite could not see: the Spanner translator emitted `TO_JSON(STRUCT(r.*))`, BigQuery-only syntax that Spanner rejects; it now emits `TO_JSON(r)`. `-Pemulator-cosmos` and `-Pemulator-dynamo` have **not** been run, so the Cosmos Patch-API and **412** assertions above remain unverified and this task stays open.
  Files: `multiclouddb-conformance/src/test/java/com/multiclouddb/conformance/us28/*.java`
- [x] T193 [US28] E2E harness: add a `patch` step to the cross-provider demonstrator between the update-verify and list steps, so patch is exercised against whichever live provider the active `*.properties` selects.
  Files: `multiclouddb-e2e/src/main/java/com/microsoft/multiclouddb/e2e/Main.java`, `multiclouddb-e2e/README.md`

**Checkpoint**: Unit coverage is separate from live-emulator certification.
`mvn test -Dtest=*PatchConformanceTest` against all three emulators is
**pending — see T192**.
