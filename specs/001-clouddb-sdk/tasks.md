...
- [x] T185 [P] [US28] Cosmos provider: `patch()` via `CosmosContainer.patchItem(...)`. Requests with a strict (`REPLACE`/`REMOVE`/`INCREMENT`) or nested path point-read and validate required-path and signed-64 numeric state. That read's ETag is attached as `If-Match` **only** for `REPLACE`, `REMOVE`, and nested non-increment ops; `INCREMENT` is exempt at every depth (`CosmosPatchOperations.increment` is atomic server-side), so concurrent increments all land instead of failing as non-retryable `CONFLICT`s. Missing path/document is `NOT_FOUND`, nonnumeric/overflow state is `INVALID_REQUEST`, and a post-validation HTTP 412 on a guarded write is `CONFLICT` without a race-prone reread. Documented residual: a *raced* increment whose target is deleted or retyped between the classifying read and the unconditional write is classified by Cosmos's native error and can surface as `INVALID_REQUEST` rather than `NOT_FOUND`.
