...
**Goal**: A provider-neutral `patch(addr, key, List<PatchOperation>)` contract with native implementations for Cosmos DB and DynamoDB, explicit `UNSUPPORTED_CAPABILITY` behavior for Spanner, and no non-transactional read-modify-write fallback. The v1 contract remains suitable for a future Spanner implementation without specializing the API around the two current providers (FR-181–FR-192).
...
- [x] T185 [P] [US28] Cosmos provider: `patch()` via `CosmosContainer.patchItem(...)`. Required-path existence, numeric type, signed-64 result bounds, and the SDK-managed item-TTL guard are encoded in one server-side filter predicate rather than a potentially stale pre-write read. Apart from the document-wide TTL guard, application-field terms are path-scoped and no `If-Match` ETag is sent, so concurrent writes to unaddressed application fields remain independent and concurrent in-range increments all land. A failed predicate (HTTP 412) or untyped `400` is classified from a point read carrying the rejection's session token: missing path/document is `NOT_FOUND`, nonnumeric/overflow state is `INVALID_REQUEST`, SDK-managed TTL is `UNSUPPORTED_CAPABILITY`, and an unexplained rejection is `CONFLICT` for 412 or the native mapping for 400.
...
- [ ] T187 [P] [US28] **Deferred to the future Spanner PATCH change.** Implement Spanner only after choosing and validating an atomic server-side design that satisfies FR-181–FR-192 without changing the provider-neutral API. The experimental implementation is preserved separately and is not shipped from this branch.
  Future files: `multiclouddb-provider-spanner/src/main/java/com/multiclouddb/provider/spanner/SpannerProviderClient.java` plus focused Spanner conformance coverage.
- [x] T188 [US28] Capability declarations: `Capability.PATCH`, `Capability.NESTED_PATCH`, `Capability.EXACT_FRACTIONAL_INCREMENT`, and `Capability.PATCH_PRESERVES_TTL` with supported/unsupported singletons; declared on all three `*Capabilities` classes. Cosmos DB and DynamoDB declare PATCH/NESTED_PATCH supported; Spanner declares both unsupported. `EXACT_FRACTIONAL_INCREMENT` is supported on DynamoDB and unsupported on Cosmos DB. `PATCH_PRESERVES_TTL` is supported on DynamoDB, unsupported on Cosmos DB, and declared unsupported on Spanner while PATCH itself is unavailable. Capability conformance asserts the provider-specific matrix instead of assuming three implementations.
...
- [x] T190 [US28] Conformance suite `us28`: provider-agnostic `PatchConformanceTest` runs for providers advertising PATCH and asserts identical outcomes for every operation type, missing-document and missing-field `NOT_FOUND`, all-or-nothing atomicity, validator rules, numeric floors/bounds, wide-integer read-back, object/null behavior, concurrent-increment safety, and the `PATCH_PRESERVES_TTL` supported/unsupported branches. Concrete PATCH subclasses ship for Cosmos DB and DynamoDB. Spanner capability conformance verifies PATCH is unsupported and that `patch()` fails with `UNSUPPORTED_CAPABILITY` before provider I/O.
...
- [ ] T192 [US28] Run the `us28` conformance suite against the live Cosmos DB and DynamoDB emulators (`-Pemulator-cosmos`, `-Pemulator-dynamo`). The unit suite is not emulator certification; in particular, verify Cosmos Patch API support and the expected **412** classification path. The prior experimental Spanner conformance run belongs to the deferred Spanner work and is not certification for this branch.
  Files: `multiclouddb-conformance/src/test/java/com/multiclouddb/conformance/us28/*.java`
- [x] T193 [US28] E2E harness: add a capability-gated `patch` step between update verification and listing. Providers advertising PATCH execute and verify the mutation; unsupported providers print a skip and continue.
...
Live Cosmos DB and DynamoDB emulator certification is **pending — see T192**.
