...
- [x] T188 [US28] Capability declarations: `Capability.PATCH` and `Capability.NESTED_PATCH` with supported/unsupported singletons; declared on all three `*Capabilities` classes. Cosmos DB and DynamoDB declare both supported; Spanner declares both unsupported. Behaviour the two implementations cannot reproduce identically — fractional `INCREMENT` accumulation and patching a TTL-bearing item — is rejected uniformly rather than declared as a capability, so no portable call stores different data depending on the provider. Capability conformance asserts the provider-specific matrix instead of assuming three implementations.
...
- [x] T190 [US28] Conformance suite `us28`: provider-agnostic `PatchConformanceTest` runs for providers advertising PATCH and asserts identical outcomes for every operation type, missing-document and missing-field `NOT_FOUND`, all-or-nothing atomicity, validator rules, numeric floors/bounds, wide-integer read-back, object/null behavior, concurrent-increment safety, and the uniform rejections (fractional `INCREMENT` delta, patch on a TTL-bearing item). Concrete PATCH subclasses ship for Cosmos DB and DynamoDB. Spanner capability conformance verifies PATCH is unsupported and that `patch()` fails with `UNSUPPORTED_CAPABILITY` before provider I/O.
