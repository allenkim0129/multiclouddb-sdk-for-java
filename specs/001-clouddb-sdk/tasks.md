...
- [x] T185 [P] [US28] Cosmos provider: `patch()` via `CosmosContainer.patchItem(...)`. Requests with a strict (`REPLACE`/`REMOVE`/`INCREMENT`) or nested path point-read and validate required-path and signed-64 numeric state. `REPLACE`, `REMOVE`, and nested non-increment ops carry a path-scoped `IS_DEFINED` filter predicate instead of an `If-Match` ETag guard; `INCREMENT` carries no predicate at any depth (`CosmosPatchOperations.increment` is atomic server-side), so concurrent increments all land instead of failing as non-retryable `CONFLICT`s. Missing path/document is `NOT_FOUND`, nonnumeric/overflow state is `INVALID_REQUEST`, and a failed path-scoped predicate (HTTP 412) is `NOT_FOUND`, since an existence check is the only precondition the request carries. Documented residual: a *raced* increment whose target is deleted or retyped between the classifying read and the unconditional write is classified by Cosmos's native error and can surface as `INVALID_REQUEST` rather than `NOT_FOUND`.
