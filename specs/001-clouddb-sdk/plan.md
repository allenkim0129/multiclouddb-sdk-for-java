...
Additionally, the SDK provides portable resource provisioning via
`ensureDatabase(String)`, `ensureContainer(ResourceAddress)`, and
`provisionSchema(Map<String, List<String>>)`. The first two methods create
individual database and collection resources. `provisionSchema` is a
higher-level bulk API that creates addressed databases in parallel, waits for
completion, then creates all containers in parallel using a bounded thread
pool. Input must respect provider addressing: one entry matching the configured
database for Spanner, and one or more logical database entries for Cosmos DB or
DynamoDB.

- **Cosmos DB** → data-plane `createDatabaseIfNotExists` + `createContainerIfNotExists` (partition key: `/partitionKey`); the caller must have database/container creation permission
...
- **Spanner** → create the configured database if absent; emulator mode also creates the configured instance, while production requires the instance to pre-exist; DDL `CREATE TABLE` uses `partitionKey STRING(MAX)`, `sortKey STRING(MAX)`, `data STRING(MAX)`, primary key `(partitionKey, sortKey)`

The Cosmos DB provider also supports `DefaultAzureCredential` as a fallback
when no account key is provided, enabling Managed Identity, Azure CLI, and
environment variable authentication. Provisioning remains on the Cosmos
data-plane SDK and succeeds only when that identity has the required resource
creation permission; the SDK does not depend on Azure Resource Manager.
...
12. Sample app data model updated to use
    `MulticloudDbKey.of(partitionKey, sortKey)` for co-location

## Technical Context

**Language/Version**: Java 17 LTS (Eclipse Adoptium Temurin-17.0.10.7-hotspot)
**Primary Dependencies**: Jackson 2.17.0, SLF4J 2.0.12, Azure Cosmos SDK 4.60.0, Azure Identity 1.12.0, AWS SDK v2 2.25.16 (DynamoDB + DynamoDB Enhanced), Google Cloud Spanner 6.62.0
...
**Scale/Scope**: ~25 new/modified Java source files, ~30 new test cases for expression translation, provisioning API across 7 files, and `DefaultAzureCredential` authentication in the Cosmos provider
...
| **1 — Thin Wrapper** | ✅ PASS | Expression translator is a lightweight string-to-string transform (< 1ms). All I/O and auth remain delegated to provider SDKs. No native-client or provider-extension API is exposed; `nativeExpression` is the explicit, typed non-portable query mode. |
...
├── QueryPage.java                  # Unchanged paging/diagnostics response; no warnings surface
...
│   └── ExpressionTranslator.java   # NEW: translate Expression AST → native string
...
multiclouddb-api/src/main/java/com/multiclouddb/spi/
...
├── CosmosProviderClient.java       # MODIFIED: use translator for portable expressions + data-plane provisioning + DefaultAzureCredential
...
**Structure Decision**: Expression types, the parser, and
`ExpressionTranslator` live in
`multiclouddb-api/src/main/java/com/multiclouddb/api/query/` (no new module
needed). The provider-client SPI, including `MulticloudDbProviderClient`, lives
in `multiclouddb-api/src/main/java/com/multiclouddb/spi/`. Each provider module
adds its own translator implementation. This reflects the shipped package
layout while preserving the original single-module dependency decision.

## Complexity Tracking

No constitution violations to justify. The design adds ~15 new types to
`multiclouddb-api`, which is consistent with the thin wrapper principle
(lightweight data types and a single parser, not a re-implementation of database
query processing). The provisioning API
(`ensureDatabase`/`ensureContainer`/`provisionSchema`) adds 3 methods to the
public client and SPI with default implementations (no-op defaults for individual
resource methods and a parallel executor for `provisionSchema`), maintaining the
thin wrapper principle by delegating directly to each provider's native
idempotent creation calls. Cosmos authentication can use
`DefaultAzureCredential`; provisioning still uses the data-plane SDK and adds no
ARM management dependency.
