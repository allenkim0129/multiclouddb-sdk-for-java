...
- Expose **field-level patch** as a first-class, capability-gated portable operation rather than leaving callers to read-modify-write. Cosmos uses `patchItem` and DynamoDB uses `UpdateItem`; both enforce the strict portable existence, numeric, and atomicity rules. Spanner declares PATCH unsupported in the current scope and fails before provider I/O. A future implementation must pass the same contract without changing the public API or falling back to a non-transactional client-side read-modify-write.
- Capability-gate **nested-path addressing** separately from patch itself. Cosmos and DynamoDB currently advertise both `PATCH` and `NESTED_PATCH`; Spanner advertises neither. Keeping the capabilities independent allows a future provider to support top-level mutation without pretending it can address sub-document paths.
- Reject **PATCH + TTL interaction** uniformly. DynamoDB's absolute `ttlExpiry` attribute would survive an `UpdateItem`, but Cosmos's relative `ttl` countdown restarts when a native patch advances `_ts`. Rather than declare a capability that lets the same call keep an expiry on one provider and change it on another, both adapters reject a TTL-bearing item with `UNSUPPORTED_CAPABILITY`.
