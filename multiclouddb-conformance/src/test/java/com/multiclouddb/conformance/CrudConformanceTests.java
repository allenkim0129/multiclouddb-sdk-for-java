...
public abstract class CrudConformanceTests {
    ...
    void deleteIdempotent() {
        assertDoesNotThrow(() -> client.delete(getAddress(), MulticloudDbKey.of("never-existed-xyz", "never-existed-xyz")));
    }
    ...
    void queryAll() {
        ...
        for (int i = 1; i <= 3; i++) client.delete(getAddress(), MulticloudDbKey.of("conf-query-" + i, "conf-query-" + i));
    }
    ...
    void queryPaging() {
        ...
        for (int i = 1; i <= 5; i++) client.delete(getAddress(), MulticloudDbKey.of("conf-page-" + i, "conf-page-" + i));
    }
    ...
    void cleanup() {
        client.delete(getAddress(), MulticloudDbKey.of("conf-test-1", "conf-test-1"));
    }
    ...
    void queryByPartitionKey() {
        ...
        for (int i = 1; i <= 3; i++) client.delete(getAddress(), MulticloudDbKey.of("alpha", "pk-alpha-" + i));
        for (int i = 1; i <= 2; i++) client.delete(getAddress(), MulticloudDbKey.of("beta", "pk-beta-" + i));
    }

    @Test @Order(12)
    @DisplayName("partitionKey null falls back to cross-partition query")
    void queryWithoutPartitionKey() {
        client.upsert(getAddress(), MulticloudDbKey.of("cross-a", "pk-cross-a"), Map.of("title", "Cross-A"));
        client.upsert(getAddress(), MulticloudDbKey.of("cross-b", "pk-cross-b"), Map.of("title", "Cross-B"));
        ...
        client.delete(getAddress(), MulticloudDbKey.of("cross-a", "pk-cross-a"));
        client.delete(getAddress(), MulticloudDbKey.of("cross-b", "pk-cross-b"));
    }
