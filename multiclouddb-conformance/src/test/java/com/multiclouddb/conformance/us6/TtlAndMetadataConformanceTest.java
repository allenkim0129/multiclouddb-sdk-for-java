...
/**
 * Conformance tests for User Story 6 — Document TTL and Write Metadata (FR-056–FR-059).
 * <p>
 * Verifies that:
 * <ul>
 *   <li>FR-056: {@link OperationOptions#ttlSeconds()} is accepted on {@code create()} without error.</li>
 *   <li>FR-056: {@link OperationOptions#ttlSeconds()} is accepted on {@code upsert()} without error.</li>
 *   <li>FR-058: {@link DocumentResult#metadata()} is populated when
 ...
 *   <li>FR-058: {@link DocumentMetadata#lastModified()} is non-null for providers that populate it.</li>
 *   <li>FR-059: Metadata is {@code null} when {@link OperationOptions#includeMetadata()} is
...
public abstract class TtlAndMetadataConformanceTest {
    ...
    // -------------------------------------------- FR-056: TTL on create()

    @Test
    @Order(1)
    @DisplayName("FR-056: create() with ttlSeconds is accepted without error")
    void createWithTtlIsAccepted() {
        MulticloudDbKey key = ConformanceHarness.uniqueKey("ttl-create");
    ...
    // -------------------------------------------- FR-056: TTL on upsert()

    @Test
    @Order(2)
    @DisplayName("FR-056: upsert() with ttlSeconds is accepted without error")
    void upsertWithTtlIsAccepted() {
        MulticloudDbKey key = ConformanceHarness.uniqueKey("ttl-upsert");
    ...
    // -------------------------------------------- FR-058: metadata is populated

    @Test
    @Order(4)
    @DisplayName("FR-058: read() with includeMetadata=true returns non-null metadata")
    ...
    @Test
    @Order(6)
    @DisplayName("FR-058: metadata.lastModified() is non-null when provider supports it")
    ...
    // -------------------------------------------- FR-059: metadata absent by default

    @Test
    @Order(7)
    @DisplayName("FR-059: read() without includeMetadata returns null metadata")
