...
public class UnsupportedCapabilityConformanceTest {
    ...
    void dynamoLikeQueryFailsFastWithStructuredError() throws Exception {
        try (MulticloudDbClient client = ConformanceHarness.createClient(ProviderId.DYNAMO)) {
            ...
            // a MulticloudDbException. If not enforced, the provider will reject it natively.
            ...
            MulticloudDbException ex = assertThrows(MulticloudDbException.class,
                    ...
                    "LIKE on DynamoDB must throw MulticloudDbException");
            assertNotNull(ex.error(), "Exception must carry a MulticloudDbError");
            // The error should indicate unsupported or invalid request
            assertTrue(
                    ex.error().category() == MulticloudDbErrorCategory.UNSUPPORTED_CAPABILITY
                            || ex.error().category() == MulticloudDbErrorCategory.INVALID_REQUEST,
    ...
    void dynamoEndsWithQueryFailsFast() throws Exception {
        try (MulticloudDbClient client = ConformanceHarness.createClient(ProviderId.DYNAMO)) {
            ...
            assertThrows(MulticloudDbException.class,
                    ...
                    "ends_with on DynamoDB must throw MulticloudDbException");
        }
    }
    ...
    void capabilityQueryPreflight() throws Exception {
        ...
        try (MulticloudDbClient client = ConformanceHarness.createClient(ProviderId.DYNAMO)) {
            ...
            try (MulticloudDbClient cosmosClient = ConformanceHarness.createClient(ProviderId.COSMOS)) {
