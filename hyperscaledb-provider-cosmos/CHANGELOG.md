# Changelog — hyperscaledb-provider-cosmos

All notable changes to the `hyperscaledb-provider-cosmos` module will be documented in this file.
...
- `CosmosProviderClient` — full `HyperscaleDbProviderClient` implementation
...
- Optional **native SDK diagnostics** pass-through when
  `HyperscaleDbClientConfig.nativeDiagnosticsEnabled()` is `true`
