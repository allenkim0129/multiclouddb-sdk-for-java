...
- `data` is reserved case-insensitively as SDK metadata across full-document
  writes and patch paths, preserving room for future provider implementations
  without narrowing the portable contract later.
...
- A top-level document field named `data` is rejected case-insensitively by
  `create`, `update`, and `upsert`; `/data` is also an invalid patch path.
  Rename application-owned fields such as `data` to `payload` before upgrading.
...
- Portable `patch(...)` implemented with the native Cosmos Patch API.
  Required paths, numeric type, integral-result bounds, and the SDK-managed TTL
...
- Portable `patch(...)` implemented as one native `UpdateItem` with compiled
  update and condition expressions. Missing documents/fields, nonnumeric
