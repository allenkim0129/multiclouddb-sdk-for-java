...
- The validating read's ETag is attached as an `If-Match` guard **only** for `REPLACE`, `REMOVE`, and nested non-increment operations. `INCREMENT` is exempt at every depth because `CosmosPatchOperations.increment` is atomic server-side, so concurrent increments all land instead of failing with a non-retryable `CONFLICT` that DynamoDB and Spanner never produce. Residual, documented trade-off: if an increment target is deleted or retyped between the classifying read and the unconditional write, Cosmos's own native error decides, so a *raced* increment can surface as `INVALID_REQUEST` rather than `NOT_FOUND`. Non-raced classification is identical on all three providers.
- Patch-specific error normalisation: the classifying pre-read returns `NOT_FOUND` for a missing document/path and `INVALID_REQUEST` for a nonnumeric target or proven overflow. A HTTP 412 after an ETag-guarded read is `CONFLICT`, without a race-prone reread. The exact Cosmos emulator status behavior remains unverified pending T192. Patch billing is workload- and indexing-dependent; no replace-equivalent or reduced-RU claim is made.
